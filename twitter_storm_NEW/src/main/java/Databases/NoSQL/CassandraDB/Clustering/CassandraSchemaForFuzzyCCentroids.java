 ...
 */
public class CassandraSchemaForFuzzyCCentroids {
    private static StringSerializer stringSerializer=new StringSerializer();

    public static String[] getCentroidsForFuzzyMeansClustering(){
        CassandraHostConfigurator cassandraHostConfigurator=new CassandraHostConfigurator("localhost:9160");
        Cluster cluster=HFactory.getOrCreateCluster("Test Cluster","localhost:9160");

        KeyspaceDefinition keyspaceDefinition=cluster.describeKeyspace("myFuzzyKeyspace1");
        if(keyspaceDefinition==null){
            ColumnFamilyDefinition columnFamilyDefinition=HFactory.createColumnFamilyDefinition("myFuzzyKeyspace1","clusters",ComparatorType.BYTESTYPE);
            KeyspaceDefinition keyspaceDefinition1=HFactory.createKeyspaceDefinition("myFuzzyKeyspace1",ThriftKsDef.DEF_STRATEGY_CLASS,1,Arrays.asList(columnFamilyDefinition));
        ...
        Keyspace keyspace=HFactory.createKeyspace("myFuzzyKeyspace1",cluster,configurableConsistencyLevel);
        String[] serializedCentroids=new String[3];String[] clusters={"cluster-0","cluster-1","cluster-2"};
        try {
    ...
    public static void setCentroidsForFuzzyMeansClustering(int index,String serializedCentroid){
        CassandraHostConfigurator cassandraHostConfigurator=new CassandraHostConfigurator("localhost:9160");
        Cluster cluster=HFactory.getOrCreateCluster("Test Cluster","localhost:9160");
